...
function solveEquation(equation) {
  // Refined regex to handle various forms of quadratic equations
  const regex = /([-+]?\d*\.?\d*)\s*\*?\s*x\^2\s*([-+]?\s*\d*\.?\d*)\s*\*?\s*x\s*([-+]?\s*\d*\.?\d*)\s*=\s*0/;
  const matches = equation.match(regex);
  if (!matches) {
    return { equation, error: 'Invalid equation format' };
  }

  const a = parseFloat(matches[1].replace(/\s+/g, '')) || 1;
  const b = parseFloat(matches[2].replace(/\s+/g, '')) || 0;
  const c = parseFloat(matches[3].replace(/\s+/g, '')) || 0;
